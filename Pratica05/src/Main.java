	...
	public static void main(String [] args) {
		...
		while(x != 0) {	
			if(x == 1) {
				...
				System.out.println("P�o Hungaro = R$ 7,00");
				...
				if(i<0) {
					System.out.println("N�o foi adicionado nenhum pedido!");
				}
				else {
					double cs = 0.0;
					...
					if(y == 1) {
						...
						double pr = cs + (cs*0.2); // define pre�o
					...
					if(y == 2) {
						...
						double pr = cs + (cs*0.4); // define pre�o
						// inicializa pizza
			...
			if(x == 4) {
				if(i >= 0) {
					...
					System.out.println("Valor Taxa: " + df.format(p[i].getVal_taxa()));
					System.out.println("Valor Pratos: " + df.format(p[i].getVal_pratos()));
					System.out.println("Valor Total: " + df.format(p[i].getVal_total()));
					df.format(1234.36);
					System.out.println("Sanduiches: ");
					p[i].imprimeSands();
					System.out.println("Pizzas: ");
